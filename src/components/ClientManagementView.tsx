...
import type { Client, Collaborator, BillingCycle } from '../types';
import {
  Users,
  Plus,
  Search,
...
export const ClientManagementView: React.FC<Props> = ({ initialSearchQuery = '' }) => {
  ...
  const loadData = async () => {
    const [c, collabs] = await Promise.all([dbService.getClients(), dbService.getCollaborators()]);
    setClients(c);
    ...
    <div className="space-y-6 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white/80 glass-panel rounded-2xl p-6 border border-gray-200">
        <div>
          ...
          <p className="text-gray-500 text-xs mt-1">Configurazione PPPoE, indirizzi IP, MAC Address e contratti di fatturazione</p>
        </div>

        <button
          onClick={handleOpenNewModal}
          className="bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white text-xs font-semibold px-4 py-3 rounded-xl shadow-lg shadow-blue-600/20 border border-blue-400/20 flex items-center justify-center gap-2 cursor-pointer transition-all shrink-0"
        >
          <Plus size={16} />
      ...
      <div className="relative max-w-md">
        ...
        <input
          ...
          className="w-full bg-white border border-gray-300 rounded-xl py-2.5 pl-11 pr-4 text-xs text-gray-900 placeholder-gray-400 focus:outline-none focus:border-blue-500 transition-all font-mono shadow-sm"
        />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {filteredClients.length === 0 ? (
          ...
          filteredClients.map(client => (
            <div
              key={client.id}
              className="glass-panel rounded-2xl p-5 border border-gray-200 hover:border-blue-300 transition-all text-xs"
            >
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-4 border-b border-gray-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-[11px] mt-0.5">{client.address || 'Indirizzo non inserito'} • CF/PIVA: {client.tax_code || 'N/D'}</p>
                  </div>
                </div>

                <div className="flex items-center gap-4 bg-gray-50 p-2.5 rounded-xl border border-gray-200 shrink-0">
                  <div>
                    <span className="text-[10px] text-gray-400 uppercase block">Canone Ricorrente</span>
                  ...
                  <div className="border-l border-gray-200 pl-4">
                    <span className="text-[10px] text-gray-400 uppercase block">Installazione</span>
                    <span className="font-mono text-xs text-gray-600">€ {client.installation_fee.toFixed(2)}</span>
                  </div>
                  {client.collaborator_name && (
                    <div className="border-l border-gray-200 pl-4">
                      <span className="text-[10px] text-gray-400 uppercase block">Collaboratore</span>
              ...
              <div className="pt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 font-mono">
                <div className="p-2.5 bg-gray-50 rounded-xl border border-gray-200">
                  <div className="text-[10px] text-gray-400 uppercase flex items-center gap-1 mb-1">
                    <Wifi size={12} className="text-cyan-600" /> Credenziali PPPoE
                  </div>
                  <div className="text-cyan-700 font-semibold text-[11px] truncate">
                    {client.pppoe_username || 'Non configurato'}
                  </div>
                  <div className="text-gray-500 text-[10px] truncate flex items-center gap-1.5">
                    ...
                    {client.pppoe_password && (
                      ...
                      >
                        {visiblePasswordIds.has(client.id) ? <EyeOff size={11} /> : <Eye size={11} />}
                      </button>
                    )}
                  </div>
                </div>
                ...
                <div className="p-2.5 bg-gray-50 rounded-xl border border-gray-200">
                  ...
                  <div className="text-gray-400 text-[10px]">Subnet /32 Static</div>
                </div>
                ...
                <div className="p-2.5 bg-gray-50 rounded-xl border border-gray-200">
                  ...
                  <div className="text-gray-700 text-[11px] font-semibold truncate">
                    {client.device_model || 'CPE WISP Standard'}
                  </div>
                  <div className="text-emerald-700 text-[10px]">{client.mac_address || '00:00:00:00:00:00'}</div>
                </div>
                ...
                <div className="p-2.5 bg-gray-50 rounded-xl border border-gray-200 font-sans">
                  ...
                  <div className="text-gray-600 text-[11px] truncate">{client.phone || client.email || 'Nessun recapito'}</div>
                  <div className="text-gray-400 text-[10px] truncate">{client.notes || 'Nessuna nota aggiuntiva'}</div>
                </div>
              </div>
            </div>
          ))
            ...
            <form onSubmit={handleSave} className="space-y-4 text-xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-gray-300 rounded-lg p-2.5 text-gray-900 uppercase font-mono"
                    />
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-gray-300 rounded-lg p-2.5 text-cyan-700 font-mono"
                    />
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full bg-white border border-gray-300 rounded-lg p-2.5 pr-10 text-gray-900 font-mono"
                      />
                      <button
                        type="button"
                      ...
                      >
                        {formPasswordVisible ? <EyeOff size={15} /> : <Eye size={15} />}
                      </button>
                    </div>
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-gray-300 rounded-lg p-2.5 text-emerald-700 font-mono uppercase"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-gray-300 rounded-lg p-2.5 text-blue-700 font-mono"
                    />
                  </div>
              ...
              <div className="bg-gray-50 p-4 rounded-2xl border border-gray-200 space-y-3">
                <h4 className="text-xs font-semibold text-emerald-700 uppercase tracking-wider flex items-center gap-1">
                  <DollarSign size={14} /> Ciclo Fatturazione & Collaboratore
                </h4>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <div className="sm:col-span-3">
                    <label className="text-gray-500 mb-1 block">Collaboratore di Riferimento (Provvigione 20%)</label>
                    <select
                      value={editingClient.collaborator_id || ''}
                      onChange={(e) => setEditingClient({ ...editingClient, collaborator_id: e.target.value ? Number(e.target.value) : null })}
                      className="w-full bg-white border border-gray-300 rounded-lg p-2.5 text-cyan-700"
                    >
