 ...
 * totali, andamento mensile, clienti assegnati e storico provvigioni.
 * Usato sia dalla modale (click su una card in Collaboratori & Provvigioni)
 * sia dalla vista self-service a pagina intera per il ruolo COLLABORATORE.
 */
export const CollaboratorDetailContent: React.FC<ContentProps> = ({ collaborator, commissions, clients, onToggleStatus }) => {
...
interface ModalProps extends Omit<ContentProps, never> {
  onClose: () => void;
}

/** Vista a 360° di un collaboratore in una modale (click su una card in Collaboratori & Provvigioni). */
export const CollaboratorDetailModal: React.FC<ModalProps> = ({ collaborator, commissions, clients, onClose, onToggleStatus }) => {
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="w-full max-w-3xl glass-panel-glow bg-white rounded-3xl p-6 border border-gray-200 max-h-[90vh] overflow-y-auto">
        ...
        <CollaboratorDetailContent collaborator={collaborator} commissions={commissions} clients={clients} onToggleStatus={onToggleStatus} />
      </div>
    </div>
  );
};
